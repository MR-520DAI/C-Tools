...
StereoCalib(const vector<string>& imagelist, Size boardSize, float squareSize, bool displayCorners = false, bool useCalibrated = true, bool showRectified = true)
{
    ...
    for (i = j = 0; i < nimages; i++)
    {
        for (k = 0; k < 2; k++)
        {
            ...
            {
                cout << "图像： " << filename << " 的分辨率不一致，请检查图像是否正确\n";
                break;
            }
            bool found = false;
            ...
            {
                for (int m = 0; m < (int)boardSize.height; m++)
                {
                    for (int n = 0; n < (int)(boardSize.width / 2); n++)
                    {
                        std::swap(corners[m * boardSize.width + n], corners[(m + 1) * boardSize.width - n - 1]);
                    }
                }
            }
            if (corners[0].y > corners.back().y)
            {
                for (int m = 0; m < (int)boardSize.width; m++)
                {
                    for (int n = 0; n < (int)(boardSize.height / 2); n++)
                    {
                        std::swap(corners[n * boardSize.width + m], corners[(boardSize.height - n - 1) * boardSize.width + m]);
                    }
                }
            }
    ...
    if (nimages < 2)
    {
        cout << "Error: 正确提取角点的图像数量过少，可能原因如下：\n";
        cout << "1.图像路径错误，未读入图像数据\n";
        cout << "2.角点数量设置错误，请检查\n";
        cout << "3.抓取的图像质量不佳，例如不够清晰、光照过暗等，导致无法有效提取角点\n";
        return;
    }
...
int main(int argc, char** argv)
{
    ...
    string imagelistfn;
    bool showRectified;
    cv::CommandLineParser parser(argc, argv, "{w|15|}{h|8|}{s|14|}{nr||}{help||}{@input|stereo_calib.xml|}");
    if (parser.has("help"))
