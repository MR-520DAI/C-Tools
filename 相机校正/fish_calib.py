...
root_dir = "E:\\data\\lingguang\\calib\\"
...
def calibrate_single(imgNums, CheckerboardSize, Nx_cor, Ny_cor, saveFile=False, saveImages=False):
    ...
    flags = cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_FILTER_QUADS + cv2.CALIB_CB_NORMALIZE_IMAGE  # 11
    ...
    for img_seq in range(imgNums):
        frame = cv2.imread(root_dir + str(img_seq) + ".png", cv2.IMREAD_GRAYSCALE)
